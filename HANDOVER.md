...
## 8. Tài liệu nghiệp vụ & thiết kế (đã đưa vào repo)
- `tai_lieu_iso/` — **toàn bộ quy trình + biểu mẫu ISO 22000** (3 bộ "Hệ thống quy trình") và các deliverable đã chuẩn hóa: bộ biểu mẫu chuẩn hóa, lịch công việc + danh mục, nội quy, hướng dẫn, rà soát, bảng định mức phụ gia, danh mục NVL… Xem `tai_lieu_iso/README.md` để biết mục lục + ánh xạ biểu mẫu → DocType.
- `tai_lieu_iso/Ban mo ta thiet ke - He thong ATTP so hoa tren ERPNext v16.md` — blueprint 6 phần + mapping 45 biểu mẫu + kịch bản 2 người dùng.
- `giaodien/` — export thiết kế Material 3 từ Google Stitch (HTML + ảnh) làm tham chiếu giao diện.
- Chưa đưa vào repo (giữ ở máy, bổ sung nếu cần): thư mục `TỰ CÔNG BỐ/` (PDF tự công bố) và `2025/` (hồ sơ thực thi theo năm).
